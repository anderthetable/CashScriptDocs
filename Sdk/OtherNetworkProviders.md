...
The ``MockNetworkProvider`` has extra methods to enable this local emulation such as ``.addUtxo()`` and ``.setBlockHeight()``. You can read more about the ``MockNetworkProvider`` and automated tests on the [testing setup](/Sdk/TestingSetup.md) page.
